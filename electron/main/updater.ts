 ...
 *
 * Update providers are configured in electron-builder.yml and resolved from
 * this fork's GitHub Releases at runtime.
 */
...
export class AppUpdater extends EventEmitter {
  ...
  constructor() {
    ...
    logger.info(
      `[Updater] Version: ${version}, channel: ${channel}, repository: ${RELEASE_REPOSITORY_OWNER}/${RELEASE_REPOSITORY_NAME}`,
    );

    autoUpdater.channel = channel;
    autoUpdater.allowPrerelease = isPrereleaseChannel(channel);
  ...
  setChannel(channel: 'stable' | 'beta' | 'dev'): void {
    ...
    autoUpdater.allowPrerelease = isPrereleaseChannel(updaterChannel);
  }
